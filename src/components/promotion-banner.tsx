  ...
  return (
    <span>
      {promotions.map((promotion, index) => (
        <span key={index}>
          {promotion.promotion}
          {promotion.booking_date && ` (예약일: ~${formatDate(promotion.booking_date)})`}
          {promotion.check_in_date && ` (투숙일: ~${formatDate(promotion.check_in_date)})`}
      ...
      <div className="container mx-auto max-w-[1440px] px-4">
        <div className="flex items-center justify-center py-3 relative">
          <div key={animationKey} className="flex items-center space-x-4">
            <div className="relative w-20 h-13 rounded-lg overflow-hidden shadow-md animate-slide-in-left">
              <Image
            ...
            <div className="h-13 flex flex-col justify-center animate-slide-in-right">
              <div className="flex items-center mb-0.5">
                <h3 className="text-lg font-bold text-white animate-typewriter">
                  {currentPromo?.property_name_ko || "프로모션 호텔"}
                </h3>
                {currentPromo?.property_name_en && (
                  <span className="text-lg font-bold text-white ml-2 animate-typewriter-delay-1">
                    {currentPromo.property_name_en}
                  </span>
                )}
              </div>
              <div className="text-base text-blue-100 animate-typewriter-delay-2">
          ...
          <div className="absolute right-4 flex items-center space-x-2">
            ...
            <button
              className="h-6 w-6 p-0 ml-2 text-white hover:bg-white/20 transition-colors"
