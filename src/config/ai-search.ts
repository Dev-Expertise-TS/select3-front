...
아래 형식으로 자세하고 실용적으로 답해줘. 각 항목은 1~3문장으로 간결하게 하되, 전체적으로 풍부하게 작성해.
1) 요약(2~3문장) — 키워드와 관련된 럭셔리 호텔/목적지 맥락, 대표 브랜드/호텔 예시 1-2개
2) 어디에 묵을까 — 우리 Select(투어비스 셀렉트) 사이트에서 제공하는 호텔 중 키워드와 관련성이 높은 2-3곳을 우선 추천하고, 각 추천에 대해 상위 브랜드/체인, 위치·특징, 추천 이유를 한 줄로 설명해. 적합한 후보가 부족하면 일반적인 상위 브랜드 대안을 제안해. 가능하면 대표 객실/스위트 팁을 한 줄 덧붙여라.
3) 언제 갈까 — 성수기/우기/베스트 시즌, 날씨·이벤트 고려사항
4) 예약 팁 — 멤버십/프로모션·조식/라운지 특전, 취소정책·업그레이드 기대치. 현재 제공 중인 혜택/프로모션이 있다면 구체적으로 명시하고, 확인 불가 시 추측하지 말고 페이지에서 확인하도록 안내
5) 목적별 포인트 — 커플/가족/호캉스/골프·스파 등 각각의 강점 한 줄씩
6) 대안 제안 — 유사 분위기의 다른 브랜드·근교 목적지 1-2개
