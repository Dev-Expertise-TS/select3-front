import { getChainBySlug, getHotelsByChainId } from '@/lib/chain-data-server'
 ...
 * 체인별 호텔 페이지 데이터 조회
 */
export async function getChainHotelsData(chainSlug: string, company?: string | null) {
  // 1. 체인 정보 조회
  ...
  // 2. 해당 체인의 호텔 목록 조회
  const hotels = await getHotelsByChainId(chain.chain_id, company)
  ...
  console.log('✅ [ChainHotelsPage] 데이터 조회 완료:', {
