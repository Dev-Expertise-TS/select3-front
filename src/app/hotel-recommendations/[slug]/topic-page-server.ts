...
export async function getTopicPageData(slug: string) {
  ...
  console.log(`[ TopicPage ] "${slug}" 페이지 조회 시작`)
  
  // 1. topic page 기본 정보 조회
  const { data: topicPage, error: pageError } = await supabase
  ...
  if (pageError || !topicPage) {
    console.error('[ TopicPage ] 페이지 조회 실패:', pageError)
    return null
  }
  
  console.log(`[ TopicPage ] 페이지 정보 조회 완료:`, topicPage.title_ko)
  
  // 2. 해당 페이지에 연결된 호텔들 조회
  const { data: pageHotels, error: hotelsError } = await supabase
    .from('select_topic_page_hotels')
    .select('*')
    .eq('page_id', topicPage.id)
    .order('pin_to_top', { ascending: false })
    .order('rank_manual', { ascending: true, nullsFirst: false })
  
  if (hotelsError) {
    console.error('[ TopicPage ] 호텔 목록 조회 실패:', hotelsError)
    return { topicPage, hotels: [] }
  }
  
  if (!pageHotels || pageHotels.length === 0) {
    console.log('[ TopicPage ] 연결된 호텔이 없음')
    return { topicPage, hotels: [] }
  }
  
  console.log(`[ TopicPage ] 연결된 호텔 ${pageHotels.length}개 발견`)
  ...
  if (hotelDetailsError) {
    console.error('[ TopicPage ] 호텔 상세 정보 조회 실패:', hotelDetailsError)
    return { topicPage, hotels: [] }
  }
  ...
  console.log(`[ TopicPage ] 최종 호텔 ${enrichedHotels.length}개 반환`)
